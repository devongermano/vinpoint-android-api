...
class Filter(context: Context) {
    ...
    fun getFilters(credentials: CPCredentials, statusResponse: CPStatusResponse? = null): Single<CPFilter> {
        val url = "$vinpointEndpoint/v1/filters/byOrg/${credentials.orgID}"
        return RequestFactory.getSecureSingle(credentials, queue, GET, url, null, CPFilter::class.java)
    }
